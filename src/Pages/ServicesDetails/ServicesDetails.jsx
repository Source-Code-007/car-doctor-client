                    ...
                    <div className="rounded-lg p-10 bg-gray-100 space-y-4">
                        <h2 className="font-bold text-3xl">Services</h2>
                        {
                            allServices.map((service, ind) => {
                                ...
                                return <NavLink key={ind} className={ ({isActive, isPending})=> `block p-5 rounded ${isActive? `bg-red-500` : 'bg-red-400'}`} to={`/services/${_id}`}><button className="w-full flex justify-between text-white"> <span>{title}</span> <FaArrowRight></FaArrowRight></button></NavLink>
                            })
                        }
                    </div>
